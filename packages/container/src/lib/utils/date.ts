...
export function getTimezones() {
  return timezones.map((timezone: Timezone) => ({
    id: timezone.tzCode,
    value: `(UTC ${timezone.utc}) ${timezone.tzCode}`,
  }));
}

export function getUserTimezone(timezones: FormattedTimezone[]) {
  const foundTimezone = timezones.find((tz: FormattedTimezone) => tz.id === USER_TIME_ZONE);
  return foundTimezone ? foundTimezone.value : '';
}

export function getTimezoneByCode(tzCode: string) {
  const foundTimezone = timezones.find((tz: Timezone) => tz.tzCode === tzCode);
  return foundTimezone ? `(UTC ${foundTimezone.utc}) ${foundTimezone.tzCode}` : '';
}
